...
void mx_print_size_or_device(t_forlong *forlong, struct stat forstat, t_flags *f, t_parse *p) {
    ...
    else if ((!S_ISCHR(forstat.st_mode) || !S_ISBLK(forstat.st_mode)) && forlong->count_of_device_files > 0 && p->count_of_dirs > 0)
        mx_print_lnumber(forstat.st_size, forlong->max_len[3] + forlong->max_len[5] + forlong->max_len[6] + 1);
    else if ((!S_ISCHR(forstat.st_mode) || !S_ISBLK(forstat.st_mode)) && forlong->count_of_device_files > 0 && p->count_of_dirs < 1)
